...
void ZenChan::Update(float elapsedSecs, std::shared_ptr<Player> player)
{
	...
	{
		m_MoveRightCommand.Execute(pPhysicsComp, pSpriteComp, m_Speed.x, 0);
	...
	if (DoRandomJumps)
	{
		...
		if (m_JumpTimer > m_JumpTime)
		{
			...
			m_MoveUpCommand.Execute(pPhysicsComp, pSpriteComp, jumpSpeed, 0);
			m_JumpTimer = 0;
			m_JumpTimeSet = false;
		}
	...
	if (m_Timer > m_ChangeDirectionTime)
	{
		...
		case 0:
			...
			break;
		case 1:
			m_MoveLeftCommand.Execute(pPhysicsComp, pSpriteComp, m_Speed.x, 0);
			break;
		}
		m_Timer = 0;
	}
